...
export const logAudit = async ({ workspaceId, action, entityId, entityType, entityTitle, user }: LogAuditProps) => {
  try {
    const { databases: adminDatabases } = await createAdminClient();
    await adminDatabases.createDocument(DATABASE_ID, AUDIT_LOGS_ID, ID.unique(), {
      workspaceId,
      action,
      entityId,
      entityType,
